    ...
    <div className="p-6 sm:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#212B1E] pb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-[#12160F] hover:bg-sage-800 border border-[#37452E] text-sage-300 font-bold rounded-xl text-xs transition-all flex items-center gap-2 disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-sm text-sage-400">Performing deep database audit...</p>
        </div>
      ) : error ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#12160F] border border-[#212B1E] rounded-3xl p-6 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4 text-xs font-mono">
              <div className="bg-[#1E2519] px-4 py-2.5 rounded-2xl border border-[#212B1E]">
                <span className="text-sage-500 text-[10px] block font-bold">Total Submissions</span>
                <span className="font-extrabold text-white text-base">{report.totalSubmissions}</span>
              </div>
              <div className="bg-[#1E2519] px-4 py-2.5 rounded-2xl border border-[#212B1E]">
                <span className="text-sage-500 text-[10px] block font-bold">Pending Reviews</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4">
            <div className="bg-[#12160F] border border-[#212B1E] p-4 rounded-2xl">
              <span className="text-[10px] text-sage-500 uppercase font-bold block">Students</span>
              <span className="text-xl font-extrabold text-white">{report.totalStudents}</span>
            </div>
            <div className="bg-[#12160F] border border-[#212B1E] p-4 rounded-2xl">
              <span className="text-[10px] text-sage-500 uppercase font-bold block">Teachers</span>
              <span className="text-xl font-extrabold text-white">{report.totalTeachers}</span>
            </div>
            <div className="bg-[#12160F] border border-[#212B1E] p-4 rounded-2xl">
              <span className="text-[10px] text-sage-500 uppercase font-bold block">Courses</span>
              <span className="text-xl font-extrabold text-white">{report.totalCourses}</span>
            </div>
            <div className="bg-[#12160F] border border-[#212B1E] p-4 rounded-2xl">
              <span className="text-[10px] text-sage-500 uppercase font-bold block">Sessions</span>
              <span className="text-xl font-extrabold text-white">{report.totalSessions}</span>
            </div>
            <div className="bg-[#12160F] border border-[#212B1E] p-4 rounded-2xl col-span-2 sm:col-span-1">
              <span className="text-[10px] text-sage-500 uppercase font-bold block">Tasks</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className={`p-4 rounded-2xl border ${report.brokenRecords > 0 ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' : 'bg-[#12160F] border-[#212B1E] text-sage-300'}`}>
            ...
            <div className={`p-4 rounded-2xl border ${report.duplicateAttempts > 0 ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' : 'bg-[#12160F] border-[#212B1E] text-sage-300'}`}>
            ...
            <div className={`p-4 rounded-2xl border ${report.orphanRecords > 0 ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' : 'bg-[#12160F] border-[#212B1E] text-sage-300'}`}>
            ...
            <div className={`p-4 rounded-2xl border ${report.invalidGrades > 0 ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' : 'bg-[#12160F] border-[#212B1E] text-sage-300'}`}>
          ...
          <div className="bg-[#12160F] border border-[#212B1E] rounded-2xl p-6 space-y-3 shadow-xl">
            ...
            {report.auditLogDetails.length === 0 ? (
              <div className="p-6 bg-[#0A0D0A] rounded-xl border border-[#212B1E] text-emerald-400 text-xs font-bold text-center flex items-center justify-center gap-2">
                <CheckCircle2 className="w-4 h-4" />
                No anomalies or database inconsistencies detected! System database is 100% healthy.
              </div>
            ) : (
              <div className="bg-[#0A0D0A] border border-[#212B1E] rounded-xl p-4 font-mono text-xs text-amber-300 space-y-1.5 max-h-60 overflow-y-auto">
                {report.auditLogDetails.map((log, idx) => (
                  <div key={idx} className="flex items-start gap-2 border-b border-[#212B1E]/50 pb-1">
                    <span className="text-sage-500 select-none">[{idx + 1}]</span>
                    <span>{log}</span>
                  </div>
                ))}
              </div>
