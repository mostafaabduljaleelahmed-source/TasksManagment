...
export const AdminTeachers: React.FC = () => {
  ...
  if (user?.role !== 'Admin') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#070907] text-white p-4">
        <div className="text-center space-y-4 max-w-md bg-[#12160F] p-8 rounded-2xl border border-red-500/20">
          <AlertCircle className="w-12 h-12 text-red-500 mx-auto" />
          <h2 className="text-xl font-bold">403 - Forbidden</h2>
          <p className="text-xs text-sage-400">Only the Academy Administrator can access this page.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#212B1E] pb-6">
        <div>
        ...
        <span className="px-3 py-1 bg-primary-500/10 border border-primary-500/30 text-primary-300 text-xs font-bold rounded-full">
          Admin Only
        </span>
      </div>
      ...
      <div className="bg-[#12160F] border border-[#212B1E] rounded-2xl p-6 shadow-xl">
        ...
        <form onSubmit={handleCreateTeacher} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full bg-[#1A2016] border border-[#37452E] text-white rounded-xl py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all ${isRtl ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
          ...
          <div>
            ...
            <div className="relative">
              <Lock className={`w-4 h-4 text-sage-500 absolute top-1/2 -translate-y-1/2 ${isRtl ? 'right-3' : 'left-3'}`} />
              <input
                type="password"
                required
                value={password}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary-600 to-primary-600 hover:from-primary-500 hover:to-primary-500 text-white font-bold rounded-xl py-3 px-4 shadow-lg shadow-primary-950/40 transition-all focus:outline-none disabled:opacity-50 text-xs"
          >
            {isSubmitting ? (
